...
export function normalizeKnowledgeBase(value: Partial<KnowledgeBaseDraft> & PublishForm): KnowledgeBaseDraft {
  ...
  return {
    ...
    actionsUrl: value.actionsUrl || '',
    lastCommitUrl: value.lastCommitUrl || '',
    lastCommitSha: value.lastCommitSha || '',
    lastPublishJobId: value.lastPublishJobId || '',
    lastPublishStatus: value.lastPublishStatus || '',
    lastPublishMessage: value.lastPublishMessage || '',
...
export function toPublishForm(kb: KnowledgeBaseDraft): PublishForm {
  return {
    ...
    knowledgeOwner: kb.knowledgeOwner,
    reviewCadence: kb.reviewCadence,
    complianceMode: kb.complianceMode,
    supportChannel: kb.supportChannel,
    escalationPath: kb.escalationPath,
